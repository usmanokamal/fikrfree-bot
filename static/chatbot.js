...
(function () {
  ...
  function formatText(text) {
    return (text || "")
      ...
      .replace(
        /^### (.*$)/gm,
        "<h3 style='font-size:1.05em;font-weight:700;margin:10px 0 6px;'>$1</h3>"
      )
      .replace(
        /^## (.*$)/gm,
        "<h2 style='font-size:1.15em;font-weight:800;margin:12px 0 6px;'>$1</h2>"
      )
      .replace(
        /^# (.*$)/gm,
        "<h1 style='font-size:1.25em;font-weight:800;margin:14px 0 8px;'>$1</h1>"
      )
      .replace(/\n/g, "<br>");
  }
              ...
              try {
                ...
                if (data.type === "content") {
                  if (!botMessageCreated) {
                    if (thinkingBubble && !thinkingRemoved) {
    ...
    const copyBtn = $("<button>")
      ...
      .click(function () {
        ...
        navigator.clipboard.writeText(plainText).then(() => {
          const prev = $(this).html();
          $(this).html("✔");
          setTimeout(() => $(this).html(prev), 900);
        });
      });
  ...
  function submitFeedback(
    ...
    container
      .find(".feedback-btn.thumbs-up, .feedback-btn.thumbs-down")
      .prop("disabled", true);
    ...
    fetch("/feedback", {
      ...
      .then(() => {
        ...
        setTimeout(() => thankYou.fadeOut(), 1600);
      })
      .catch(() => {
        container
          .find(".feedback-btn.thumbs-up, .feedback-btn.thumbs-down")
  ...
  function detectLanguage(text) {
    ...
    if (total <= 4) {
      if (ruCount >= 2) return "roman_urdu";
      if (enRatio > 0.5) return "english";
      return "roman_urdu";
    }
    if (ruCount >= 2) return "roman_urdu";
    if (enRatio > 0.35) return "english";
    return "english";
  }
    ...
    fetch("/translate", {
      ...
      .then((data) => {
        if (data.status === "success") {
          ...
          if (target === "roman_urdu")
            buttonElement
              .html("EN")
              .attr("data-current-lang", "roman_urdu")
              .attr("title", "Translate to English");
          else
            buttonElement
              .html("Roman Urdu")
      ...
      .catch(() => {
        buttonElement
          .prop("disabled", false)
          .html(target === "roman_urdu" ? "EN" : "Roman Urdu");
      });
  }
  ...
  $(document).ready(function () {
    ...
    setTimeout(() => {
      appendMessage(
        "Hi! I’m the FikrFree Assistant. Ask me about insurance plans, claims, doctor consultations, or e‑pharmacy.",
        "bot"
      );
    }, 400);
    ...
    if (SpeechRecognition) {
      ...
      $("#mic-btn").on("click", function () {
        alert(
          "Voice input is not supported in this browser. Try Chrome/Edge over HTTPS or localhost."
        );
      });
    }
  });
